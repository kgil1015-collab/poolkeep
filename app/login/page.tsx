    ...
    <div className="min-h-screen flex items-center justify-center px-4" style={{background:'linear-gradient(172deg,#e8f5ff 0%,#c4dff5 38%,#9ac4e8 70%,#6ba8d8 100%)'}}>
      <div className="w-full max-w-sm bg-white rounded-3xl shadow-2xl p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <Link href="/forgot-password" className="text-xs text-pool-dark font-medium hover:underline">Forgot password?</Link>
            </div>
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-base outline-none focus:border-pool-dark focus:ring-2 focus:ring-pool-dark/10 transition-all bg-surface" />
          </div>
        ...
        <p className="text-center text-[11px] text-text-faint mt-6">
          <Link href="/privacy" className="hover:text-text-muted underline">Privacy Policy</Link>
          {' · '}
          <Link href="/terms" className="hover:text-text-muted underline">Terms of Service</Link>
        </p>
      </div>
    </div>
  )
