...
export function calculateRecommendations(
  ...
  if (test.ph === null) { recs.push(MISSING.ph) }
    ...
    recs.push({ status: phSlightlyHighNeedsAction ? 'action' : 'monitor', param: 'ph', title: isSalt ? 'Lower pH — routine salt pool maintenance' : phSlightlyHighNeedsAction ? 'Lower pH before shocking' : 'pH slightly high', desc: isSalt ? `pH is at ${test.ph}. A small acid dose will bring it into range. Salt generators push pH upward over time — regular acid additions are a normal part of salt pool maintenance.` : phSlightlyHighNeedsAction ? `pH is at ${test.ph} — lower it to 7.2 before shocking. At this pH, most of the shock you add is wasted. Add muriatic acid first, then shock.` : `pH is at ${test.ph}. A small acid dose will bring it into range.`, tags: [`Muriatic Acid (or Dry Acid) · ${acidAmount(dose)}`, isSalt ? 'Re-test tomorrow' : phSlightlyHighNeedsAction ? 'Do this before shocking' : 'Monitor daily'] })
  } else if (test.free_chlorine !== null && test.free_chlorine < 0.5 && test.ph > 7.2) {
    recs.push({ status: 'action', param: 'ph', title: 'Lower pH to 7.2 before shocking', desc: `pH at ${test.ph} is in range for normal use, but lower it to 7.2 first — at higher pH most of the shock you add is wasted. Add muriatic acid first, then shock.`, tags: [`Muriatic Acid (or Dry Acid)`, 'Do this before shocking'] })
  } else {
