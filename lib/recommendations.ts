  ...
  if (fc !== null && fc < 0.5) {
    ...
    if (isSalt) {
      ...
      raw.push({
        ...
        amount: needsAcid
          ? taHigh
            ? `${acidAmount(acidDose)}\n${liquidDose} gal\n${oz(dose, 'lbs')} (${Math.ceil(dose)} × 1-lb bag${Math.ceil(dose) !== 1 ? 's' : ''})\nPoint a return jet toward the water surface — run 2–4 hrs`
            : `${acidAmount(acidDose)}\n${liquidDose} gal\n${oz(dose, 'lbs')} (${Math.ceil(dose)} × 1-lb bag${Math.ceil(dose) !== 1 ? 's' : ''})`
          : `${liquidDose} gal\n${oz(dose, 'lbs')} (${Math.ceil(dose)} × 1-lb bag${Math.ceil(dose) !== 1 ? 's' : ''})`,
        why: `Free chlorine is at ${fc} ppm — water is unsafe to swim in. Here is something most pool owners never learn: the effectiveness of chlorine is almost entirely controlled by pH. Chlorine exists in two forms in water — active (hypochlorous acid, HOCl) and inactive (hypochlorite ion, OCl⁻). Only the active form kills bacteria and algae. At pH 7.0, about 73% of your chlorine is in that active form. At pH 7.5, it drops to 49%. At pH 7.8, only 33%. At pH 8.0, just 21%. ${phHigh && phEfficiency ? `Your current pH of ${ph} means only about ${phEfficiency} of the chlorine you add will actually be working. Lowering pH first means 2–3× more active sanitizer from the same amount of product.` : phUnknown ? `Since pH is untested, add a small acid dose first as a precaution — if your pH is elevated you could waste the majority of the chlorine you add.` : `With pH already in range, a high percentage of the chlorine you add will be in its active, sanitizing form.`}${cyaLow ? ` CYA (stabilizer) is ${cya === null ? 'untested' : `at ${cya} ppm — below the effective range`}. Without stabilizer protecting it, UV sunlight destroys chlorine within hours. The dose shown is higher than usual to account for this — but the real fix is getting CYA into the 30–50 ppm range so future chlorine actually holds.` : cyaHigh ? ` CYA at ${cya} ppm is elevated — stabilizer at high levels partially binds chlorine and reduces how much stays "free" and active. A higher dose is needed to push past this and reach effective sanitizing levels.` : ''}`,
        how: `${needsAcid
          ? acidHow
          : `Step 1 — Brush all pool surfaces — walls, floor, steps, and any corners. Algae clings to surfaces and chlorine cannot sanitize what it cannot contact. Brushing first makes the treatment dramatically more effective.\n\nStep 2 — `}Add chlorine in the evening — UV sunlight destroys chlorine rapidly during the day. Use ${oz(dose, 'lbs')} of granular cal-hypo (65%) — or ${liquidDose} gal of liquid chlorine (10–12.5%). Pour around the perimeter with the pump running.`,
...
export function calculateRecommendations(
  ...
  const MISSING: Record<string, Rec> = {
    ...
    calcium_hardness: { status: 'unknown', param: 'calcium_hardness', title: 'Water hardness not tested', desc: `Low hardness causes water to leach calcium from pool surfaces, leading to etching over time. High hardness causes scale deposits. Strips measure total hardness — that number works fine here.${isSalt ? ' For salt pools, scale on the salt cell reduces chlorine output and shortens cell life.' : ''}`, tags: [`Ideal: ${isSalt ? '200–350' : '200–400'} ppm`, 'Test monthly'] },
  }
  ...
  if (test.free_chlorine === null) { recs.push(MISSING.free_chlorine) }
    ...
    if (isSalt) {
      ...
      const phNeedsWork = test.ph !== null && test.ph > 7.2
      if (phNeedsWork) {
        recs.push({ status: 'action', param: 'chlorine', title: 'Chlorine critically low — two steps', desc: `Free chlorine at ${test.free_chlorine} ppm — unsafe for swimming. Step 1: add muriatic acid to bring pH to 7.2. Step 2: add chlorine. At pH ${test.ph}, high pH makes chlorine far less effective — lower it first and the same dose works 2–3× better.`, tags: [] })
      } else {
        recs.push({ status: 'action', param: 'chlorine', title: 'Chlorine critically low — add chlorine tonight', desc: `Free chlorine at ${test.free_chlorine} ppm — unsafe for swimming. Add liquid chlorine or granular shock this evening.`, tags: [] })
      }
    }
